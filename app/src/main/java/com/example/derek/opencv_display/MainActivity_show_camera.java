...
/*
        int rawWidth = mRgba.cols();
        int IDEAL_WIDTH = 512;
        double scaleFactor;
        if (rawWidth > IDEAL_WIDTH) {
            scaleFactor = (double)IDEAL_WIDTH / rawWidth;
            Size newSize = new Size( (double)IDEAL_WIDTH, (int)round( (double)mRgba.rows() * (double)IDEAL_WIDTH / (double)mRgba.cols() ));
            Imgproc.resize(mRgba, mRgba, newSize, 0, 0, 0);
        }
        else scaleFactor = 1;
        ...
        Mat SE1 = new Mat(8, 16, CvType.CV_8U, seVal);
